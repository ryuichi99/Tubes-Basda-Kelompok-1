...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add') {
    ...
    $stmt = $conn->prepare("SELECT id FROM kategori WHERE nama_kategori = ? AND user_id = ?");
    $stmt->bind_param('si', $nama_kategori, $uid);
    $stmt->execute();
    $res = $stmt->get_result();
    ...
    $ins = $conn->prepare("INSERT INTO kategori (user_id, nama_kategori, batas, periode, created_at) VALUES (?, ?, ?, ?, ?)");
    $ins->bind_param('isiss', $uid, $nama_kategori, $batas, $periode, $now);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update') {
    ...
    $stmt = $conn->prepare("SELECT id FROM kategori WHERE nama_kategori = ? AND user_id = ? AND id != ?");
    $stmt->bind_param('sii', $nama_kategori, $uid, $id);
    $stmt->execute();
    $res = $stmt->get_result();
    ...
    $stmt = $conn->prepare("UPDATE kategori SET nama_kategori = ?, batas = ?, periode = ? WHERE id = ? AND user_id = ?");
    $stmt->bind_param('sissi', $nama_kategori, $batas, $periode, $id, $uid);
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) as count FROM transaksi WHERE kategori_id = ? AND user_id = ?");
...
if (isset($_GET['edit'])) {
    $id = (int)$_GET['edit'];
    $stmt = $conn->prepare("SELECT * FROM kategori WHERE id = ? AND user_id = ?");
    $stmt->bind_param('ii', $id, $uid);
    $stmt->execute();
    $editData = $stmt->get_result()->fetch_assoc();
}

// Ambil data kategori user
$categories = [];
$stmt = $conn->prepare("SELECT id, nama_kategori, batas, periode, created_at FROM kategori WHERE user_id = ? ORDER BY created_at DESC");
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <?php foreach ($categories as $cat): ?>
          <div class="p-5 bg-ungu-gelap rounded-xl shadow-md border border-ungu-terang/50 hover:border-tosca-terang/50 transition duration-300">
            ...
            <div class="mt-4 pt-3 border-t border-ungu-terang/30">
              <p class="text-xs text-white/50">
                Batas: <?= formatCurrency($cat['batas']) ?> per <?= h($cat['periode']) ?>
              </p>
            </div>
          </div>
        <?php endforeach; ?>
